...
rm -f ./absolutely-not-samurai/*.so && \
...
```bash
export SAM2_REPO_ROOT=/path/to/absolutely-not-samurai  # path to this repo
...
```python
def get_extensions():
    srcs = ["absolutely-not-samurai/csrc/connected_components.cu"]
    compile_args = {
        "cxx": [],
        "nvcc": [
            "-DCUDA_HAS_FP16=1",
    ...
    ext_modules = [CUDAExtension("absolutely-not-samurai._C", srcs, extra_compile_args=compile_args)]
    return ext_modules
```
</details>
